    ...
    public class TcpCommsClient : TcpCommsBase
    {
        ...
        public ConnectionResponse Open(string address, int port, bool secure = false)
        {
            try
            {
                ...
                return SetUpTcpClient(address, secure);
            }
            catch (Exception)
            {
                //TODO: log this issue
        ...
        public (string XmlSafeId, int InvokeId) GetInvokeId()
        {
            lock (invokeIdLock)
            {
                if (invokeId == 9999)
                ...
                return (result.ToString().PadLeft(INVOKE_ID_LEN, '0'), result);
            }
        }
        ...
        /// This is almost identical to the example project
        /// </summary>
        int RawSendRequest(string request)
        {
            /*
            ...
            try
            {
                ...
                // HEADER : INVOKE ID
                var (XmlSafeId, InvokeId) = GetInvokeId();
        ...
        public (string Response, int InvokeId) ReadXMLMessage(bool addWait = false)
        {
            ...
            try
            {
                ...
                var responseInvokeIdString = new string(reader.ReadChars(INVOKE_ID_LEN));
